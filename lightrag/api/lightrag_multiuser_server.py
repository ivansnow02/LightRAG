...
from lightrag.api.routers.document_routes import (
    DocumentManager, create_document_routes_with_multi_user,
)
...
SUPPORTED_BINDINGS = {
    ...
    "langchain_gemini": ("lightrag.llm.langchain_gemini", ["langchain_gemini_complete", None]),  # Gemini没有内置embedding
...
async def lifespan(app: FastAPI):
        ...
        if args.llm_binding == "openai":
            ...
            async def openai_wrapper(prompt, **kwargs):
                return await openai_complete_if_cache(
                    args.llm_model, prompt, base_url=args.llm_binding_host, api_key=args.llm_binding_api_key, **kwargs
                )
    ...
    if args.embedding_binding == "openai":
        ...
        if func_name:
            ...
            raise ValueError(f"Embedding function not supported or found for binding: {args.embedding_binding}")
    else:
    ...
    rag_factory_config = {
        ...
        "llm_model_kwargs"                   : {
            "host"   : args.llm_binding_host, "timeout": args.timeout, "options": {"num_ctx": args.max_tokens},
            "api_key": args.llm_binding_api_key,
        } if args.llm_binding in ["lollms", "ollama"] else {
            "timeout": args.timeout} if args.llm_binding == "azure_openai" else {},
...
app.include_router(create_query_routes_with_multi_user(global_args.top_k), prefix="/api")
